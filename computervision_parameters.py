DATASET_numberOfEntries = 200

EXPORTEDJSON_trainingPath = "trainingData.json"
EXPORTEDJSON_validationPath = "validationData.json"

EXPORTEDJSON_filepath = {'training': EXPORTEDJSON_trainingPath, 'validation': EXPORTEDJSON_validationPath}

IMAGES_trainingPath = "train_images"
IMAGES_validationPath = "validation_images"

IMAGES_filepath = {'training': IMAGES_trainingPath, 'validation': IMAGES_validationPath}

"""
IMAGES_shoeTrainingPath = f"{IMAGES_trainingPath}/shoe"
IMAGES_shoeValidationPath = f"{IMAGES_validationPath}/shoe"

IMAGES_shoepath = {'training': IMAGES_shoeTrainingPath, 'validation': IMAGES_shoeValidationPath}
"""
RAWDATA_trainingPath = "iMaterialist/fgvc4_iMat.train.data.json"
RAWDATA_validationPath = "iMaterialist/fgvc4_iMat.validation.data.json"

RAWDATA_filepath = {'training': RAWDATA_trainingPath, 'validation': RAWDATA_validationPath}

RAWDATA_taskMapPath = "iMaterialist/fgvc4_iMat.task_map.json"
RAWDATA_labelMapPath = "iMaterialist/fgvc4_iMat.label_map.json"

CV_MODELS_APPAREL_filename = 'apparelModel'

CV_MODELS_SHOE_TO_GENDER_filename = 'shoeToGenderModel'

CV_MODELS_SHOE_AUTOENCODER_filename = 'shoeAutoencoder'

MISC_models_path = 'trainedModels'
